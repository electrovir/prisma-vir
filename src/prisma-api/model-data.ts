 ...
 * array of `PrismaAllModelsCreate` for sequential data creation.
 *
 * @category Internal
 * @example
 *
 * ```ts
 * import {PrismaAddModelData} from 'prisma-vir';
 ...
 * ```
 */
export type PrismaAddModelData<
...
>({
    ...
    data: IsAny<PrismaClient> extends true ? any : PrismaAddModelData<PrismaClient, TypeMap>;
}): Promise<void> {
...
export async function dumpData<const TypeMap extends BaseTypeMap>({
    prismaClient,
    ...options
}: Readonly<PartialWithUndefined<PrismaDataDumpOptions>> & {
    ...
    const data: Partial<Record<PrismaModelName<TypeMap>, AnyObject[]>> = await arrayToObject(
        ...
        async (modelName) => {
            try {
                ...
                const isRelationDoesNotExistError =
                    errorMessage.startsWith('relation') && errorMessage.includes('does not exist');
                const isOrderbyMissingError =
                    errorMessage.includes('argument') &&
                    errorMessage.includes('orderby') &&
                    errorMessage.includes('missing');

                /* node:coverage disable */
                if (isOrderbyMissingError || isRelationDoesNotExistError) {
                    /**
